...
class ApiModel extends Model
{
    ...
    public static function interAttadenceMapping($lmId,$bm_id,$basicData)
    {
        ...
        ## Check Entry Exist ##
            $leadDetailsAgainstEvent=ApiModel::getLeadMaster($lemmId,$bm_id,$basicData);;
    ...
    public static function getLeadMaster($lemmId,$bm_id,$basicData)
    {
        ...
        ## Genrate OTP ##
            ...
            $datar->where('lemm.lemm_id', $lemmId);
            $datar->where('lemm.aem_id', $aem_id);
            $datar->where('lemm.bm_id', $aem_id);
    ...
    public static function ActivityHere($allRequestData)
    {
        ...
        if(!empty($allRequestData['exhid']) && !empty($allRequestData['activeId'])){
            ...
            $user = ApiModel::getLeadDataById($allRequestData['username']);
    ...
    public static function getLeadDataById($username)
    {
        $emailId = $username;
    ...
    {
        $detail = DB::table('1_journey_video_master')->where('jvm_id',1)->first();
        if($detail)
        {
           return $detail->jvm_video_url;
        }
        else{
            return '';
        }
    ...
    public static function GetMentorByCatId($mcm_id)
    {
         ...
         $imageurl='https://'.request()->getHost().config('app.rootFolder').'/se/public/assets/images/mentors/';
         $videourl='https://'.request()->getHost().config('app.rootFolder').'/se/public/assets/videos/';
         $asseturl='https://'.request()->getHost().config('app.rootFolder').'/se/public/assets/';
