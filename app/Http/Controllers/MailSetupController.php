...
// dd($eventDetails);
        return view('emailer.mailsetup',['data'=>$eventDetails,'prefix_url' => $this->getBaseUrl]);
    }
    public function mailstp(Request $request)
    {
        $profileDetail=Session('AprofileDetail');
          $eventDetails=Session('selectedEvent'); 
         $data = EventMaster::where('aem_id',$request->aem_id)->first();
    // dd($data);
        return view('emailer.mailsetup',['data'=>$data,'prefix_url' => $this->getBaseUrl]);
    }
