...
class LobbyController extends Controller
{
    ...
    public function managelobby()
    {
        ...
        $leadList=DB::table('1_lobby_banner_master as bm')
        ...
        $category = DB::table('1_lobby_banner_category_master')->where('bcm_status','Y')->get();
        
        $bannerCount = DB::table('1_lobby_banner_master as bm')
                        ->Join('1_lobby_banner_category_master as bcm', 'bm.bcm_id','bcm.bcm_id')
                        ->Select('bcm.bcm_name',DB::raw('count(*) as total'))
                        ->groupBy('bm.bcm_id')
                        ->get();
        
        return view('managelobby.index',[
              'leadList'=>$res,
              'category'=>$category,
    ...
    public function AddBanner(Request $request) {
        ...
        if(!null==request('upload_photo') && request('photoupload')=='photoupload'){
                ...
                $imagePath='/assets/images/lobby/banners/cat-'.request('egcategory').'/';
        ...
        {
            DB::table('1_lobby_banner_master')
        ...
        {
            $bmId = DB::table('1_lobby_banner_master')->insertGetId($setArray);
        }
        
        $page = !empty(request('page')) ? '?page='.request('page') : '';
        return redirect($this->getBaseUrl.'/managelobby'.$page);
    }
    ...
    public function EditBanner(Request $request)
    {
        ...
        $list = DB::table('1_lobby_banner_master')
